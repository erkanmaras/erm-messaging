...
namespace Erm.Messaging.MessageGateway.InMemory.Tests;

public class InMemoryMessageStatusRegistryTests
{
    [Theory]
    ...
    {
        var registry = CreateInMemoryMessageStatusRegistry();
    ...
    public async Task EntryDates_ShouldSetFrom_IClock()
    {
        ...
        var registry = new InMemoryMessageStatusRegistry(clock.Object);
        var messageId = Uuid.Next();
        await registry.MarkAsProcessing(messageId);
    ...
    private static InMemoryMessageStatusRegistry CreateInMemoryMessageStatusRegistry()
    {
        var clock = new Mock<IClock>();
        clock.Setup(o => o.UtcNow).Returns(DateTimeOffset.UtcNow);
        return new InMemoryMessageStatusRegistry(clock.Object);
    }
}